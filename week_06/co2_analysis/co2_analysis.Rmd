...
Let's first load the data. From the comments included in this dataset, the original columns are:
> Year, month, decimal date, monthly CO2 average, deseasonalized monthly CO2 average, number of days in month, std. dev of days, unc. of monthly mean
...
While ARIMA cannot be used with non-stationary data, auto.arima() will perform the following steps before using ARIMA:
> 1. Check for stationarity with ADF
> 2. If non-stationary, apply differencing with enough steps to pass ADF
> 3. Apply ARIMA
