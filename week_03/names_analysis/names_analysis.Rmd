...
To make this analysis easier, I'll remove useless values such as "_PRENOMS_RARES" from the dataset. In addition, The annais field will be converted to a proper date format, which should make visualization clearer with GGPLOT.

## Drop useless values and fix formatting in the dataframe
```{r}
firstNames = subset(firstNames, preusuel != '_PRENOMS_RARES' & annais != 'XXXX')
```

```{r}
firstNames$annais <- as.POSIXct(firstNames$annais, format="%Y")
```
...
## 1. Frequency of the name Mario over time
```{r}
marioFrequency = firstNames %>%
                  filter(preusuel == "MARIO") %>%
...
```{r}
ggplot(data=marioFrequency, aes(x=annais, y=nombre, group=1)) +
  geom_line() +
  geom_point()
```

## 2. Most frequent firstname by gender
```{r}
